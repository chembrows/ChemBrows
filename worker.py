...
import hosts
import functions
from companies.finder_company import FinderCompany
from log import MyLog
from exceptions import *

# Import MyWindow only for type checking, cyclic import otherwise
...
class Worker(QtCore.QThread):
    ...
    def __init__(self, parent: Optional["MyWindow"] = None):
        ...
        if parent is None:
            ...
            # Where to store the graphical abstracts
            self.PATH = parent.DATA_PATH + "/graphical_abstracts/"
    ...
    def run(self):
        ...
        finder_company = FinderCompany(self.url_feed)

        try:
            finder_company.init_feed()
            finder_company.init_infos(self.parent.dict_journals)
        except WorkerError:
            self.l.error("Exiting worker, problem w/ the feed")
            self.parent.list_failed_rss.append(self.url_feed)
            return

        company_handler = finder_company.find_company_handler()
        print(company_handler)

        # self.l.info("{}: {}".format(company_obj.journal, len(company_obj.feed.entries)))
...
if __name__ == "__main__":
    worker = Worker()
    worker.url_feed = "http://feeds.feedburner.com/acs/jcisd8"
    worker.run()
    pass
